...
const DirectorDash = () => (
  <DashShell role="DIRECTRICE" user="Mme Akpovi"
    ...
    <PageHeader greeting="Bonjour Mme Akpovi 👋" sub="Mardi 5 mai · Trimestre 2 — semaine 8 sur 12">
      ...
      <Card padding={20}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-tertiary)', margin: '4px 0 0' }}>Trimestre 2 · objectif 95%</p>
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
            <Badge variant="brand">Trim. 2</Badge>
            <Badge variant="neutral">Trim. 1</Badge>
          </div>
        </div>
        <BarChart/>
  ...
  <DashShell role="PARENT" user="M. Hounsou"
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 16 }}>
      <Card padding={0} style={{ overflow: 'hidden' }}>
        ...
        <div>
          {[
            { name: 'Aïcha · Trim. 1', amount: '125 000', due: 'Payé · 12 oct.', state: 'paid' },
            { name: 'Mathieu · Trim. 1', amount: '95 000', due: 'Payé · 12 oct.', state: 'paid' },
            { name: 'Aïcha · Trim. 2', amount: '125 000', due: 'Échéance dans 6 jours', state: 'due' },
            { name: 'Mathieu · Trim. 2', amount: '95 000', due: 'Échéance dans 6 jours', state: 'due' },
          ].map((p, i) => (
...
const StudentDash = () => (
  <DashShell role="ÉLÈVE" user="Aïcha Hounsou"
    ...
    <PageHeader greeting="Salut Aïcha 👋" sub="3ᵉ A · Tu es 4ᵉ de la classe ce trimestre. Belle progression !">
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 18 }}>
      <Card style={{ background: 'linear-gradient(135deg, var(--brand-800), var(--brand-600))', color: '#fff', border: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6, fontSize: 12 }}>
          <Icon name="arrowUp" size={12}/> +0,6 pts vs trim. 1
        </div>
      </Card>
      <Card>

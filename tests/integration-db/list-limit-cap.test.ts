 ...
 * Attendu : limite plafonnée à 100, valeurs par défaut si la saisie est invalide.
 */
type Handler = Parameters<typeof callRoute>[0];

// metaKey : objet de pagination de la réponse (finance/payments renvoie `meta`).
const ROUTES: Array<{ name: string; handler: Handler; path: string; sizeKey: "limit" | "pageSize"; defaultSize: number; metaKey?: "meta" }> = [
  { name: "appointments", handler: appointments, path: "/api/appointments", sizeKey: "limit", defaultSize: 20 },
  { name: "audit-logs", handler: auditLogs, path: "/api/audit-logs", sizeKey: "limit", defaultSize: 50 },
  ...
  { name: "incidents", handler: incidents, path: "/api/incidents", sizeKey: "limit", defaultSize: 20 },
...
describe("N16 — limite de page plafonnée et saisie invalide tolérée", () => {
  for (const route of ROUTES) {
    it(`${route.name} : ?${route.sizeKey}=100000 est plafonné à 100`, async () => {
      ...
      expect((res.body as Record<string, Record<string, number>>)[route.metaKey ?? "pagination"][route.sizeKey]).toBe(100);
    });

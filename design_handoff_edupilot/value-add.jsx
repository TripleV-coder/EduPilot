...
const AcademicConfig = () => {
  // Lit & écrit le système global via data-period sur <html>
  // pour rester en phase avec la bascule du panneau Tweaks.
  const [system, setSystemLocal] = React.useState(() =>
    document.documentElement.getAttribute('data-period') || 'trimestre'
  );
  React.useEffect(() => {
    const obs = new MutationObserver(() => {
      const v = document.documentElement.getAttribute('data-period') || 'trimestre';
      setSystemLocal(prev => prev === v ? prev : v);
    });
    obs.observe(document.documentElement, { attributes: true, attributeFilter: ['data-period'] });
    return () => obs.disconnect();
  }, []);
  const setSystem = (v) => {
    setSystemLocal(v);
    document.documentElement.setAttribute('data-period', v);
    // Persiste via le mécanisme Tweaks
    try { window.parent.postMessage({ type: '__edit_mode_set_keys', edits: { periodSystem: v } }, '*'); } catch (e) {}
  };
  return (

...
| N28 | Élevée | Le cache mémoire (`MemoryCache`, `lib/cache/redis.ts`) — **cache de production** (instance unique sans Upstash, décision du propriétaire) — est une `Map` sans borne ; une entrée expirée n'est retirée qu'à la relecture de la même clé. Les clés portent l'URL, ses paramètres (curseur, recherche) et l'utilisateur, les valeurs la réponse entière : la mémoire du serveur croît jusqu'au redémarrage (famille M4/N1) | 3 | En cours | — | `tests/lib/cache/memory-cache.test.ts` | RSS 338 → 545 Mo après la série de mesures (critère < 500 Mo) | — |

...
| N12 | Faible | `src/lib/services/analytics/helpers.ts` duplique les builders de `analytics-dashboard/` et n'est importé nulle part (code mort) | 8 | Corrigé | `0aa07c0` | `grep` des imports dans src/tests/e2e/scripts → 0 ; tsc, eslint, 2 959 tests verts après suppression | 472 lignes sans appelant | fichier supprimé |

...
## Lot 3 — Performance et bornage (en cours)
...
1. ⏳ `finance/stats` : plans de paiement et encaissements agrégés en SQL (caractérisation écrite : `tests/integration-db/finance-stats.test.ts`).
2. ✅ N8 (`ab8aa4c`, `761d460`).
3. Migration des routes encore paginées par offset vers le curseur (`?page=` toléré jusqu'au Lot 8) : 4 faites (`959ce87`), 17 restantes (`announcements`, `appointments`, `audit-logs`, `classes`, `compliance/data-requests`, `courses`, `events`, `finance/payments`, `homework`, `incidents`, `messages`, `organizations`, `payments`, `root/logs`, `root/organizations`, `root/schools`, `root/users`, `schools`). La plupart de leurs consommateurs chargent une liste pour un sélecteur ou un filtrage dans le navigateur (`?limit=50…500`, sans contrôle de page) : même règle « aucune troncature fonctionnelle ».
4. Index justifiés par `EXPLAIN ANALYZE`, par migration.
5. Batterie du Lot 3 : smoke (1 Mo / 1 s), `latency.mjs`, RSS < 500 Mo, Lighthouse page Notes < 2 Mo, E2E `grades-flow`, `tsc`, lint, vitest, build, intégration.
...
| C3 | Critique | Endpoints non paginés (évaluations, statistiques, schedules, fees, health, scholarships, analytics) + N1 | 3 | En cours (Lot 3) — évaluations, statistiques, listes, santé, bourses, tableaux de bord et BI corrigés ; batterie finale à passer | `0356461` `20d56d1` `9ae0515` `6965078` `083f228` `7364584` `e6e7725` `8ed0a34` `93f633f` `b62db28` | smoke seuils 1 Mo/1 s, latency, Lighthouse, RSS | 35 violations ; 100 Mo ; 8,7 Go | smoke 3a : 3 violations ; tableaux de bord p95 226 / 161 ms ; overview p95 165 ms ; RSS 457 Mo |
...
| M5 | Moyenne | 163/231 `findMany` sans `take` | 3 | En cours (Lot 3) — revue faite, N+1 corrigés, 5 des 6 occurrences non bornées traitées (reste `finance/stats`) | `618933d` `0d3a609` `82baca8` `d926558` `9eb7657` `8d2c446` `b62db28` | revue + plafond helper | 163 | voir « Revue M5 » (Lot 3) |
...
| N1 | Critique | Épuisement mémoire (voir ci-dessus) | 3 | Corrigé pour `/api/evaluations` (source des 100 Mo) ; RSS à confirmer par la batterie | `0356461` | RSS | 8 746 Mo | RSS 457 Mo après 90 requêtes analytiques (5 710 Mo au Lot 0) |
...
| N13 | Moyenne | RSS du serveur 221 → 457 Mo après 90 requêtes analytiques séquentielles (plafond du Lot 3 : 500 Mo) | 3 | À surveiller par la batterie | — | `latency.mjs` + `/proc/<pid>/status` | 457 Mo | — |

...
import { getRootUserRoleClass } from "@/lib/ui/status-styles";
...
export default function RootUsersPage() {
    ...
    const suspendedUsersCount = users.length - activeUsersCount;
        ...
        <PageGuard roles={["SUPER_ADMIN"]}>
            <div className="space-y-6 max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <MetricCardPro label="Comptes chargés" value={users.length} hint="Fenêtre actuelle de résultats" icon={Users} tone="primary" />
                    <MetricCardPro label="Actifs" value={activeUsersCount} hint="Utilisateurs opérationnels" icon={Activity} tone="success" />
                    <MetricCardPro label="Suspendus" value={suspendedUsersCount} hint="Nécessitent une revue" icon={ShieldCheck} tone="warning" />
                </div>

                <SectionToolbar
                    title="Recherche globale"
                    description="Affinez par nom, e-mail ou rôle pour accélérer les actions root."
                    leading={
                        <div className="relative flex-1 min-w-[280px]">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                            <Input
                                aria-label="Rechercher un utilisateur global"
                ...
                <Card className="p-4 rounded-xl shadow-sm border border-border">
                    ...
                    {isLoading ? (
                        ...
                        <div className="h-24 grid place-items-center text-destructive text-sm">
                        ...
                        <EmptyStateAction
                            ...
                            description="Ajustez votre recherche ou élargissez la requête pour retrouver un profil."
                                        ...
                                        <TableRow key={user.id} className="hover:bg-muted/30 transition-colors">
                                            ...
                                            <TableCell>
                                                {user.role === "SUPER_ADMIN" ? (
                                                    <Badge variant="outline" className={`font-bold gap-1 shadow-none ${getRootUserRoleClass(user.role)}`}>
                                                        <ShieldCheck className="w-3 h-3" /> ADMIN SYSTÈME
                                                    </Badge>
                                                ) : user.role === "SCHOOL_ADMIN" ? (
                                                    <Badge variant="outline" className={`shadow-none ${getRootUserRoleClass(user.role)}`}>
                                                        ADMIN ÉTABLISSEMENT
                                                    </Badge>
                                                ) : (
                                            ...
                                            <TableCell className="text-sm text-muted-foreground">
                                                <div className="flex flex-col">
                                                    <span>{formatDateTimeShort(user.updatedAt)}</span>

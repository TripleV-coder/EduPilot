...
export async function GET(request: NextRequest) {
  try {
    ...
    if (incidentType) where.incidentType = incidentType as Prisma.BehaviorIncidentWhereInput["incidentType"];
    if (severity) where.severity = severity as Prisma.BehaviorIncidentWhereInput["severity"];
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Notify parents
    const parentNotifications = incident.student.parentLinks.map(link => ({

...
export const POST = createApiHandler(
    async (request, context) => {
        try {
            ...
            const rate = await checkRateLimit(strictLimiter, `broadcast:${context.session.user.id}`);
            if (!rate.success) {
            ...
            if (context.session.user.role !== "SUPER_ADMIN" && classRecord.schoolId !== getActiveSchoolId(context.session)) {
            ...
            if (context.session.user.role === "TEACHER") {
                const teachesClass = await prisma.classSubject.findFirst({
                    where: { classId, teacher: { userId: context.session.user.id } },
                    select: { id: true },
                });
                if (!teachesClass) {
                    return NextResponse.json(

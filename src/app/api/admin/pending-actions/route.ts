...
export async function GET() {
  try {
    ...
    // const pendingDataRequests = await prisma.complianceRequest.count({
    //   where: {
    //     status: "PENDING",
    //   },
    // }).catch(() => 0); // Fallback if model doesn't exist
    const pendingDataRequests = 0;
    ...
    // Count compliance reports to review
    const pendingComplianceReports = 0; // TODO: Add when compliance report model exists

...
import { Prisma, ProfessionCategory } from "@prisma/client";
...
export const GET = createApiHandler(
  async (request, _context) => {
    try {
      ...
      if (category) {
        where.category = category as ProfessionCategory;
      }
  ...
  async (request, _context) => {
    try {
      ...
      const profession = await prisma.profession.create({
        data: {
          name,
          category: category as ProfessionCategory,
          description,
        },
      });
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === "P2002") {
          return NextResponse.json({ error: "Cette profession existe déjà" }, { status: 409 });
        }
      }

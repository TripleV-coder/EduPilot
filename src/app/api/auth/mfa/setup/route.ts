...
import { generateSecret, generateQRCode, verifyToken, generateBackupCodes, hashBackupCodes, encryptSecret } from "@/lib/auth/two-factor";
import { z } from "zod";
...
export const POST = createApiHandler(
    async (request, { session }, _t) => {
        ...
        if (action === "enable") {
            ...
            // Générer les backup codes en clair (affichés une seule fois à l'utilisateur)
            const plainBackupCodes = generateBackupCodes();
            // Hacher les backup codes avant sauvegarde en DB
            const hashedBackupCodes = await hashBackupCodes(plainBackupCodes);
            // Chiffrement AES-256-GCM du secret avant sauvegarde
            ...
            return NextResponse.json({
                ...
                // Retourner les codes en clair : affichés une seule fois, non stockés en clair

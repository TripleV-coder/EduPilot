...
export async function POST(req: NextRequest) {
  try {
    ...
    const rateLimitResult = checkRateLimit(rateLimitKey, FORGOT_PASSWORD_RATE_LIMIT);
    ...
    // 2. Vérifier le mot de passe temporaire (si mode manuel)
    if (!validated.useMagicLink) {
      ...
      // Le tempPassword est maintenant hashé dans la DB
      ...
      const isCorrectTemp = tokenRecord.tempPassword
        ? await bcrypt.compare(normalizedInput, tokenRecord.tempPassword)
        : false;

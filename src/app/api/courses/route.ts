...
export async function GET(request: NextRequest) {
  try {
    ...
    // Non-SUPER_ADMIN must only see courses from their school
    ...
    const [courses, total] = await Promise.all([
      prisma.course.findMany({
        ...
        include: {
          ...
          modules: {
            take: 50, // Limit to prevent N+1
            orderBy: { order: "asc" },
            include: {
              lessons: {
                take: 100, // Limit lessons per module
                orderBy: { order: "asc" },
                select: {
                  id: true,
                  title: true,
    ...
    return createPaginatedResponse(courses, page, limit, total);
  } catch (error) {

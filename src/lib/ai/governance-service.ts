...
export function hasExternalAIConfigured() {
  return appEnv.ai.enabled && appEnv.ai.hasExternalKeys;
}

class GovernanceService {
  async execute(request: GovernanceRequest, startTime: number): Promise<GovernanceResponse> {
  ...
  async analyzeClass(request: GovernanceRequest): Promise<GovernanceActionResult> {
  ...
  async analyzeSchool(request: GovernanceRequest): Promise<GovernanceActionResult> {
  ...
  async predictGrades(request: GovernanceRequest): Promise<GovernanceActionResult> {
    const studentId = this.resolveRequestedStudentId(request);
    if (!studentId) {

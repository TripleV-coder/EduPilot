...
export class UserCreationService {
   ...
   */
  private async createTeacherProfile(tx: Prisma.TransactionClient, userId: string, data: CreateUserData) {
    const profile = await tx.teacherProfile.create({
      data: {
        userId,
        schoolId: data.schoolId!,
        // Le modèle Prisma utilise `matricule` (pas `employeeNumber`)
        matricule: data.teacherData?.employeeNumber || generateEmployeeNumber('T'),
   ...
   */
  private async createStudentProfile(tx: Prisma.TransactionClient, userId: string, data: CreateUserData) {
    if (!data.studentData) {
   ...
   * Créer profil parent
   */
  private async createParentProfile(tx: Prisma.TransactionClient, userId: string, data: CreateUserData) {
